...
> The complete reference: every way to install, every command and flag, the
> full configuration surface, and how the pieces fit together. Just arrived?
> The [README](../README.md) gets you sending a file in two minutes.
...
> identity, and a **desktop GUI** (Tauri 2, macOS/Windows/Linux) that drives the
> same daemon. Relay federation and mobile are planned (see Roadmap).
...
**macOS, Windows and Linux** are supported — the daemon IPC is a unix socket, a
named pipe on Windows.

## Quickstart

> New here? [`docs/QUICKSTART.md`](QUICKSTART.md) walks through standing up a
...
Five nouns cover everything a send produces or a receive takes: a **ticket**
(the `.arvolo` file / `arvc…` string — pure P2P), a **code** (short, dictable,
P2P via rendezvous), a **link** (browser URL on the relay), a **mailbox send**
(`arvm…`, sealed to one recipient on the relay), and a **waiting offer** (a
send addressed to you, not yet taken — shown with its 8-hex handle). Listing
commands take `--json` for scripts; every artefact is printed **alone on
stdout**, narration goes to stderr, so piping always yields exactly the thing.
...
| `arvolo history [--json]` | What already **happened**: the whole log of finished transfers, newest first (pipe it — `arvolo history \| head`). Read-only — nothing here can still be acted on, which is what separates it from `status`. |
...
| `arvolo daemon start\|run\|stop\|status [--json]` | Stay reachable **always**, as a background service: nobody is at the keyboard, so it decides from your trust settings and notifies you about the rest. Also the local control socket every other command drives. `start` spawns it in the background, `run` keeps it in this terminal (systemd/launchd), `stop` shuts it down and keeps it down, `status` says what it's doing. See [`docs/DAEMON.md`](DAEMON.md). Note: the bare CLI daemon's own desktop notifications don't reach the screen on macOS 26+ (Apple requires a signed app bundle for that) — parked offers still show in `arvolo status`, and the desktop app notifies normally. |
...
| `arvolo device pair\|join\|sync\|status [--json]` | Use arvolo on more than one device: `pair` shows a code, `join` takes it (sharing one identity), `sync` propagates the address book, `status` reports fingerprint and last sync. See [Multiple devices](#multiple-devices). |
...
[`docs/PROTOCOL.md`](PROTOCOL.md); for the security rationale, see
...
`relay`. See [`docs/DEPLOY.md`](DEPLOY.md) (`relay/` ships a Dockerfile +
`docker-compose.yml`).
...
Shipped recently: the **desktop GUI** on all three platforms (with a signed and
notarized macOS build), **multi-device** support on one shared identity (paired
over a short code, address book kept in step), the always-open client
(`listen`/`daemon`), and browser download links (the Firefox Send heir).
Planned next: relay federation (short codes across independent relays) and
mobile. Post-MVP ideas are tracked in
[`docs/ROADMAP-FUTURE.md`](ROADMAP-FUTURE.md).
