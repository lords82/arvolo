...
| `arvolo daemon start\|run\|stop\|status [--json]` | Stay reachable **always**, as a background service: nobody is at the keyboard, so it decides from your trust settings and notifies you about the rest. Also the local control socket every other command drives. `start` spawns it in the background, `run` keeps it in this terminal (systemd/launchd), `stop` shuts it down and keeps it down, `status` says what it's doing. See [`docs/DAEMON.md`](DAEMON.md). |
